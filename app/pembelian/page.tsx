...
function Pembelian() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const tanggalISO = `${yyyy}-${mm}-${dd}`; // 🟢 untuk summary
      ...
      // cek riwayat existing untuk bikin id unik
      const res = await fetch(
        `https://platif-ai-default-rtdb.asia-southeast1.firebasedatabase.app/rwytSPBU1/${jenisBBM}.json`
      );
      ...
      const data = {
        ...
        liter,
        nominal,
        tanggal: tanggalISO, // 🟢 simpan tanggal di transaksi
        waktu: waktuStr,
        idPembelian,
      };
      ...
      await fetch(
        `https://platif-ai-default-rtdb.asia-southeast1.firebasedatabase.app/rwytSPBU1/${jenisBBM}/${idPembelian}.json`,
        {
          method: "PUT",
      ...
      // 🟢 update summary harian untuk Analytics (rebuild full dari riwayat)
      type Transaksi = {
        plat: string;
        jenisBBM: string;
        liter: number;
      ...
      const summaryPath = `https://platif-ai-default-rtdb.asia-southeast1.firebasedatabase.app/Summary/${tanggalISO}/${jenisBBM}.json`;

      // 1. Ambil semua riwayat transaksi untuk jenisBBM & tanggal ini
      const riwayatPath = `https://platif-ai-default-rtdb.asia-southeast1.firebasedatabase.app/rwytSPBU1/${jenisBBM}.json`;
      const riwayatRes = await fetch(riwayatPath);
      const riwayatAll: Record<string, Transaksi> = await riwayatRes.json();

      // filter hanya transaksi pada tanggal yang sama
      const transaksiHariIni = Object.values(riwayatAll || {}).filter(
        (trx) => trx.tanggal === tanggalISO
      );

      // 2. Hitung ulang summary dari nol
      let totalTransaksi = 0;
      let totalLiter = 0;
      let totalNominal = 0;
      const pembeliSet = new Set();

      for (const trx of transaksiHariIni) {
        totalTransaksi += 1;
        totalLiter += trx.liter;
        totalNominal += trx.nominal;
        pembeliSet.add(trx.plat.replace(/\s+/g, "").toUpperCase()); // unik per plat
      }

      // 3. Buat summary baru
      const updatedSummary = {
        totalTransaksi,
        totalLiter: Number(totalLiter.toFixed(2)),
        totalNominal,
        totalPembeli: pembeliSet.size, // jumlah plat unik
      };

      // 4. Simpan summary
      await fetch(summaryPath, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      <nav className="fixed top-0 left-0 w-full bg-white shadow-md z-50">
        <div className="flex items-center justify-between py-3 px-4 md:px-6 w-full">
          ...
          <div className="flex items-center space-x-4 ml-5">
            <img src="/Platif-AI.png" alt="Logo 1" className="h-10 w-full object-contain" />
...
export default withRole(Pembelian, ["operator"]);